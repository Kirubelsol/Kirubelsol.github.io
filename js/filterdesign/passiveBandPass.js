//Displaying Second order Band pass RLC filter

document.querySelector('.js-RLCbandpassbtn').addEventListener('click', BandPassRLC);

function BandPassRLC(){
  
  let R = parseFloat(document.querySelector('.js-resistanceBPRLC').value)
  let R_unit = document.querySelector('.js-resistanceBPRLC-unit').value
  let C = parseFloat(document.querySelector('.js-capacitanceBPRLC').value)
  let C_unit = document.querySelector('.js-capacitanceBPRLC-unit').value
  let L = parseFloat(document.querySelector('.js-inductance-BPRLC').value)
  let L_unit = document.querySelector('.js-inductance-BPRLC-unit').value
  let Wc1 = parseFloat(document.querySelector('.js-cutoffBPRLC1').value)
  let Wc1_unit = document.querySelector('.js-cutoffBPRLC1-unit').value
  let Wc2 = parseFloat(document.querySelector('.js-cutoffBPRLC2').value)
  let Wc2_unit = document.querySelector('.js-cutoffBPRLC2-unit').value
  ...
  if (((R && C && L) || (R && C && Wc1) || (R && C && Wc2) || (R && L && Wc1) || (R && L && Wc2) || (R && Wc1 && Wc2) || (C && L && Wc1) || (C && L && Wc2) || (C && Wc1 && Wc2) || (L && Wc1 && Wc2)) && !( R && L && C && Wc1 && Wc2) && !( R && L && C && Wc1) && !( R && L && C && Wc2) && !( R && C && Wc1 && Wc2) && !(L && C && Wc1 && Wc2)) {
    ...
    if(invalid.length === 0){
      ...
      let cutoffDerivation = `
        ...
        <p class ="serif"> The cutoff frequency is the frequency at which the magnitude of \\( Vout \\over Vin \\) is often defined to be below \\( {-3dB =} {1 \\over \\sqrt{2}} \\). In band-pass filter we have upper and lower cutoff frequencies. <p>
      ...
      if (R && L && C){
        ...
        SolutionElement = `
          ...
          \\( \\bullet \\; w_l = ${rounding( Wc1)} {rad \\over sec}  \\) &nbsp; and &nbsp; \\( w_u = ${rounding( Wc2 )}  {rad \\over sec} \\) <br> 
      ...
      `
        ...
        \\( = {1 \\over {${rounding(L)}({${rounding(Wc1)}}^2) + ${rounding(R)}(${rounding(Wc1)}) }} \\approx ${rounding(1/(Wc1*Wc1*L + R*Wc1))}\\) F<br>
      ...
      `
        ...
        \\( \\bullet \\; \\) Using the value of R and the  \\(  w_l \\) equation: \\( w_l = - {{${rounding(R)} \\over {2(${rounding(L)})} } + \\sqrt { ({${rounding(R)} \\over {2(${rounding(L)}})})^2 + {1 \\over {(${rounding(L)})(${rounding(C)})} } } }  {rad \\over sec}  \\approx ${rounding( (-R/(2*L)) + math.sqrt( math.square(R/(2*L)) + (1/(L*C))) )} {rad \\over sec} \\) <br>
        \\( \\bullet \\;  f_l =  {{w_l} \\over {2\\pi}} \\) .Hence  \\( f_l\\approx ${ rounding(Wc1 /(Math.PI*2))} \\) &nbsp; Hz
      ...
      `
        ...
        \\( \\bullet \\; \\) Using the equation of \\(  w_u \\) and rearranging it: \\( R = { w_uL -  {1 \\over {w_uC}} }\\)
        \\( = { ${rounding(Wc2)}(${rounding(L)}) - {1 \\over {${rounding(Wc2)}(${rounding(C)})}} } \\approx ${rounding(-1/(Wc2*C) + L*Wc2)} \\Omega \\)
      ...
      if ( (R <= 0) || (C <= 0) || (L <= 0) || (Wc1 <= 0) || (Wc2 <= 0)){
        ...
        <div class = "container-lg mt-5">
          ...
          <p class ="serif">A band-pass filter can be designed using a series connection of resistor, capacitor, and inductor. The output voltage is taken across the resistor. <br> Your input values result in invalid results such as negative values. Please choose correct input values. If you want to learn more on how the band-pass filter works, you can get in on content tab. Thanks. </p>
        </div>
        `
      }
        ...
        <div class = "container-lg mt-5">
          ...
          <div class="text-center d-none d-xl-block mb-3">
            <span class="tt" data-bs-placement="bottom" title="RLC Band Pass Filter Schematic">
              <img src="../img/filterdesign/RLCbandpass.PNG" class = "rounded" width = "55%" height = "55%" alt="RLC Band Pass Filter">
            </span>
            <p class="subtitles">RLC band-pass filter schematic</p>
          </div>
          ...
          <p class ="serif"> Center Frequency \\( (w_o) \\) refers to the midpoint of the pass band. Most of the time the arthimetic mean is used while at times geometric mean is used. </p>
          ...
          <p class ="serif"> Quality factor (Q) is a dimensionless parameter indicating the performance of the filter. It is the ratio of the center frequency to the band width. A higher value indicates narrow pass band of frequencies resulting in sharper notch while lower value indicates wider pass band region.  </p>
          ...
          <ul style="list-style-type:none; line-height: 2.5;" class = "mb-4 georgia-font fst-italic">  
            ...
            \\( \\bullet \\; \\frac{V_o}{V_i} =   { {${rounding(R)} \\over ${rounding(L)}}s \\over { {s}^2 + { ${rounding(R)} \\over ${rounding(L)}} s + {1 \\over {(${rounding(L)})(${rounding(C)})}} } } = { ${rounding(R/L)}s \\over { {s}^2 + {${rounding(R/L)}} s +  {${rounding(L*C)}}}  } \\) 
          </ul>
          ...
          <ul style="list-style-type:none; line-height: 2.5;" class = "mb-4 georgia-font fst-italic">
            ...
            \\( = { {${rounding(R/L)}}(w) \\over \\sqrt { {{{${rounding(R/L)}}(w)}^2} +  ({{${rounding(1/(L*C))}} - {w}^2})^2 }} \\)</li>
          ...
          <ul style="list-style-type:none; line-height: 2.5;" class = "mb-4 georgia-font fst-italic">   
            ...
            \\( \\bullet \\; \\phi = {\\pi \\over {2}} - tan^{-1} ({ {R \\over L}w  \\over {{1 \\over {LC}} - {w}^2} }) \\\cdot ({180 \\over \\pi}) &deg = {\\pi \\over {2}} - tan^{-1} ({ {${rounding(R)} \\over ${rounding(L)}}w  \\over {{1 \\over {(${rounding(L)})(${rounding(C)})}} - {w}^2} }) \\\cdot ({180 \\over \\pi}) &deg  \\) <br>
            \\( \\bullet \\; \\phi = {\\pi \\over {2}} - tan^{-1} ({ {${rounding(R/L)}}w  \\over {{ {${rounding(1/(L*C))}}} - {w}^2} }) \\\cdot ({180 \\over \\pi}) &deg   \\) <br> 
          ...
          <p class ="serif"> Total impedance indicates the combined effect of the passive elements (capacitors, resistors, and inductors) in a circuit, and itaffects the gain and phase shift of the circuit. </p>
          <ul style="list-style-type:none; line-height: 2.5;" class = "mb-4 georgia-font fst-italic">    
  ...
  //if only one input or all three inputs
    ...
    <div class = "container-lg mt-5">
      ...
      <p class ="serif" >  You have entered insufficient, exccess, or invalid inputs. Please enter only three valid values for successful calculation. </p>
    </div>`;
